...
static
int pscom_name_is_lower(const char name1[8], const char name2[8])
{
	return memcmp(name1, name2, 8) < 0;
}

static
void pscom_ondemand_cleanup(pscom_con_t *con)
{
...
static
void pscom_ondemand_direct_connect(pscom_con_t *con)
{
...
static
void pscom_ondemand_close(pscom_con_t *con)
{
	pscom_ondemand_cleanup(con);
}
				       ...
				       int nodeid, int portno, const char name[8])
{
	...
	memcpy(con->pub.remote_con_info.name, name, sizeof(con->pub.remote_con_info.name));
